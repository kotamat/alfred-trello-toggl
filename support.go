...
func getRunningTimer() (toggl.TimeEntry, bool) {
	for _, entry := range cache.Toggl.Data.TimeEntries {
		if entry.IsRunning() {
			return entry, true
		}
	}
...
func getProjectsById() map[int]toggl.Project {
	...
	for _, proj := range cache.Toggl.Data.Projects {
...
func findTimerByName(name string) (toggl.TimeEntry, bool) {
	log.Println(name)
	for _, entry := range cache.Toggl.Data.TimeEntries[:] {
		log.Println(entry.Description)
...
func findTagByName(name string) (toggl.Tag, bool) {
	for _, tag := range cache.Toggl.Data.Tags {
		if tag.Name == name {
			return tag, true
		}
	}
...
func getCardsForQuery(query string) []trello.Card {
	entries := cache.Trello.Data.TimeEntries[:]
	matchQuery := strings.ToLower(query)
	matched := []trello.Card{}

	for _, entry := range entries {
		if strings.Contains(strings.ToLower(entry.Name), matchQuery) {
...
func getTimeEntriesForQuery(query string) []toggl.TimeEntry {
	entries := cache.Toggl.Data.TimeEntries[:]
	matchQuery := strings.ToLower(query)
